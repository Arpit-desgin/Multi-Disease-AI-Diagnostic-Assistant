...
def test_embeddings():
    ...
    try:
        ...
        em = EmbeddingModel()
        logger.info("✅ EmbeddingModel initialized using all-MiniLM-L6-v2")
        ...
        if len(embedding) != 384:
            logger.error(f"❌ Wrong embedding dimension: expected 384, got {len(embedding)}")
            return False
        ...
        if len(embeddings) != len(test_texts):
            logger.error("❌ Batch embedding count mismatch")
            return False
        
        logger.info("✅ TEST 2 PASSED: Embeddings working correctly")
        return True
...
def test_retrieval():
    ...
    try:
        ...
        if not all_success:
            logger.warning("⚠️  TEST 3 PARTIAL: Some queries returned no results")
            logger.warning("   → Check: 1) Documents indexed? 2) Data exists?")
        else:
            logger.info("✅ TEST 3 PASSED: Retrieval working")
...
def test_rag_generation():
    ...
    try:
        ...
        logger.info("✅ RAG Pipeline initialized (fully local, no API needed)")
        ...
        for query, disease_type in test_queries:
            ...
            try:
                ...
                if result.get("error"):
                    ...
                    print(f"  📚 Sources: {len(result['sources'])} identified")
        ...
        if not all_success:
            logger.warning("⚠️  TEST 4 PARTIAL: Some queries failed")
        else:
            logger.info("✅ TEST 4 PASSED: RAG generation working")
...
def test_embedding_consistency():
    ...
    try:
        ...
        if distance < 1e-6:
            ...
            logger.info("✅ TEST 5 PASSED: Embedding model is consistent")
            return True
        else:
            print(f"  ⚠️  Minor variance detected (distance: {distance})")
            logger.warning("⚠️  Embeddings are not perfectly consistent")
            return False
